...
class ReservableItemController extends Controller
{
    ...
    public function show(Request $uten) {
    ...
    public function store(Request $request) {
        $validator = Validator::make($request->all(), [
            ...
            'type' => Rule::in(["wahsing_machine", "room"]),
                ...
                function (string $attribute, mixed $value, \Closure $fail) {
                    ...
                    foreach ($minutes as $minute) {
                        if (!is_numeric($minute) || intval($minute) < 0 || intval($minute) > 59) {
                            $fail("Invalid \"allowed starting minutes\" list (there is a value that is either not a number or not between 0 and 59)!");
                        }
                    }
                }
            ],
