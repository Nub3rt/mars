...
class ReservableItemFactory extends Factory
{
     ...
     * Note: only creates rooms
     *
     * @return array<string, mixed>
     */

    private const DEFAULT_RESERVATION_DURATIONS = [15, 30, 45, 60, 90, 120, 180];

    public function definition()
    {
        $out_of_order_from = $this->faker->boolean(50) ? Carbon::now() -> add(-2, 'hour') : null;
        $out_of_order_until = is_null($out_of_order_from) ? null :
            ($this->fake->boolean(50) ? Carbon::now()->add(random_int(-50,50), 'minute') : null);
